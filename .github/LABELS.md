...
- **Manual sync**: Trigger the workflow manually via GitHub Actions > Sync Labels > Run workflow, or use the GitHub CLI commands shown above.
...
4. **Don't create too many labels** - aim for 30-40 total
5. **Archive unused labels** rather than delete (consider dependencies)

## Label Count

The repository maintains **38 labels** across 10 categories - enough for good organization without overwhelming contributors. Check `labels.yml` for the current complete list.
